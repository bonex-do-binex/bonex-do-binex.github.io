...
scene.add(new THREE.HemisphereLight(0x3344ff, 0x050510, 0.5));
...
function createAsphaltMaterial() {
    ...
    return new THREE.MeshStandardMaterial({ map: tex, roughness: 0.75, metalness: 0.15, color: 0x181818 });
}
...
const laneMat = new THREE.MeshStandardMaterial({ color: 0xf5f5f5, emissive: 0xf5f5f5, emissiveIntensity: 0.6 });
const lane = new THREE.Mesh(new THREE.PlaneGeometry(0.4, 20000), laneMat);
lane.rotation.x = -Math.PI / 2;
lane.position.y = 0.001;
scene.add(lane);

const sideLineGeo = new THREE.PlaneGeometry(0.3, 20000);
const sideLineMat = new THREE.MeshStandardMaterial({ color: 0xfff2c0, emissive: 0xfff2c0, emissiveIntensity: 0.4 });
...
const grass = new THREE.Mesh(new THREE.PlaneGeometry(20000, 20000), new THREE.MeshStandardMaterial({ color: 0x1f3a1b, roughness: 1 }));
...
for (let i = 0; i < 350; i++) {
    ...
    const mat = new THREE.MeshStandardMaterial({
        ...
        metalness: 0.9, roughness: 0.45
    });
    const b = new THREE.Mesh(buildingGeo, mat);
    b.scale.set(w, h, w);
    const side = Math.random() > 0.5 ? 1 : -1;
    b.position.set(side * (260 + Math.random() * 900), h / 2, (Math.random() - 0.5) * 20000);
    b.castShadow = true;
    scene.add(b);
}

// --- STREETLIGHTS (FIXED HIERARCHY) ---
const poleGeo = new THREE.CylinderGeometry(0.12, 0.16, 7, 8);
const lampGeo = new THREE.SphereGeometry(0.25, 10, 10);
const streetLights = [];

for (let i = 0; i < 90; i++) {
    const zBase = -9000 + i * 220;
    [ {x: -14, z: zBase}, {x: 14, z: zBase + 110} ].forEach(pos => {
        const group = new THREE.Group();
        const pole = new THREE.Mesh(poleGeo, new THREE.MeshStandardMaterial({ color: 0xb0b0b0 }));
        pole.position.y = 3.5;
        group.add(pole);
        
        const lamp = new THREE.Mesh(lampGeo, new THREE.MeshStandardMaterial({ color: 0x222222, emissive: 0xfff2c0, emissiveIntensity: 4 }));
        lamp.position.set(pos.x > 0 ? 1.6 : -1.6, 6.3, 0);
        group.add(lamp);

        group.position.set(pos.x, 0, pos.z);
        scene.add(group);
        streetLights.push({ group, lamp });
    });
}

// --- CAR ---
const car = new THREE.Group();
const paintMat = new THREE.MeshPhysicalMaterial({ color: 0x990000, metalness: 0.85, roughness: 0.18, clearcoat: 1.0 });
const body = new THREE.Mesh(new THREE.BoxGeometry(4.2, 0.9, 9.5), paintMat);
body.position.y = 0.9;
body.castShadow = true;
car.add(body);

const cabin = new THREE.Mesh(new THREE.BoxGeometry(3.6, 0.9, 4.5), new THREE.MeshPhysicalMaterial({ color: 0x000000, transparent: true, opacity: 0.8 }));
...
// Headlights
const headMat = new THREE.MeshStandardMaterial({ color: 0x111111, emissive: 0x88ccff, emissiveIntensity: 3 });
[[1.1, 1.0, 4.7], [-1.1, 1.0, 4.7]].forEach(p => {
    const h = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.35, 0.15), headMat);
    h.position.set(...p);
    car.add(h);
...
const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.6, 0.7, 0.85);
...
function animate() {
    ...
    const targetCamPos = car.position.clone().add(new THREE.Vector3(0, 6, -18).applyQuaternion(car.quaternion));
